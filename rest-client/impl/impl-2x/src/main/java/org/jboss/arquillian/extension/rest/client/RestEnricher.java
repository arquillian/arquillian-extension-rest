            ...
            for (int i = 0; i < parameterTypes.length; i++) {
                ...
                for (Annotation annotation : parameterAnnotations) {
                    if (annotation instanceof ArquillianResteasyResource) {
                        final String resourcePath = ((ArquillianResteasyResource) annotation).value();
                        if (ClientRequest.class.isAssignableFrom(parameterTypes[i])) {
                            values[i] = new ClientRequestFactory(getBaseURL()).createRelativeRequest(resourcePath);
                        } else {
